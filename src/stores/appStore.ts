...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  generateHandoverSummary: () => {
    ...
    const inProgressAlertIds = alerts
      .filter(a => a.status === 'acknowledged' || a.status === 'active')
      .map(a => a.id);
    const inProgressEvents = get().events.filter(e => e.alertId && inProgressAlertIds.includes(e.alertId));
